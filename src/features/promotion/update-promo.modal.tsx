...
export default NiceModal.create(({ promo }: Props) => {
  const modal = useModal()

  const queryClient = useQueryClient()

  const [form, setForm] = useState<UpdatePayloadState>({
    id: '',
    code: '',
    minimumPurchase: '',
    discount: '',
  ...
  const addPromoMutation = useMutation<{ message: string }, Error, UpdatePayloadState>(
    updatePromo,
    {
      onSuccess: () => {
  ...
  useEffect(() => {
    ...
    setForm({
      id: promo?._id,
      code: promo?.code || '',
      minimumPurchase: String(promo?.minimumPurchase) || '',
      discount: String(promo?.discount) || '',
      maximumDiscount: String(promo?.maximumDiscount) || '',
      validFrom: promo?.validFrom || moment().toDate(),
      validTill: promo?.validTill || moment().add(1, 'days').toDate(),
      discountFor: promo?.discountFor || 'all',
      selectedUsers: promo?.selectedUsers || [],
    })
  }, [promo])
  ...
  const submitHandler = (e: _FormSubmitEvent) => {
    e.preventDefault()
    if (!promo?._id) return toast.error('Promo id is required')
    if (
      !form.code ||
    ...
    toast.promise(addPromoMutation.mutateAsync(form), {
      ...
      success: (res) => res.message ?? 'Promo updated successfully',
      error: (err) => err.message ?? 'Failed to update promo',
    })
  }
      ...
      <form onSubmit={submitHandler}>
        <div className='grid grid-cols-2 gap-8 mb-6'>
          <div className='flex flex-col gap-5'>
            ...
            />
            {promo?.minimumPurchase}|{promo?.maximumDiscount}
            <DatePickerInput
        ...
        <div>
          ...
          <Input
            ...
            type='radio'
            value='selectedUsers'
            checked={form.discountFor === 'selectedUsers'}
            name='discountFor'
            onChange={changeHandler}
            label={
              <>
